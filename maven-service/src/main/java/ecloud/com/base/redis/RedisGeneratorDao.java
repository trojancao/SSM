...
public class RedisGeneratorDao<K extends Serializable, V extends Serializable> {
    ...
    @Autowired
    @Qualifier("redisTemplate")
    protected RedisTemplate<K, V> redisTemplateSerializable;
     ...
     * @return
     */
    public Object getSequence(String key) {
        return redisTemplate.opsForValue().increment(key, 1);
    }

    public Object getValue(String tableKey, String hashKey) {
        return redisTemplate.opsForHash().get(tableKey, hashKey);
    }
    ...
    /**
     * 获得缓存的list对象 @Title: range @Description:
     * TODO(这里用一句话描述这个方法的作用) @param @param key @param @param start @param @param
    ...
    public V getValue(final byte[] key) {
        return redisTemplateSerializable.execute(new RedisCallback<V>() {
