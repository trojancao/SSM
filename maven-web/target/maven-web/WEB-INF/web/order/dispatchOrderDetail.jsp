			...
			<form id="orderfm" method="post" role="orderfm"
				...
				<div class="modal-body">
					...
					<div class="modal-footer">
						...
						<button type="button"
							class="btn btn-primary dispatchOrder"
							onclick="save()">派单</button>
						<button type="button"
							class="btn btn-warning cancel"
							onclick="openDeleteModel()">撤销</button>
					</div>
				</div>
			</form>
		</div>
...
//            detailView:true,
            ...
            onClickCell: function(field, value, row, $element){
            	if("action" == field && (row.state=='01' || row.state=='02' || row.state=='04')){
            		...
            		$("#fmMemo").val(row.memo);
            		$("#fmOrderDetailId").val(row.orderDetailId);
            		...
            		$(".dispatchOrder").css("display", "none");
            		if(row.state=='01'){
	...
	function openAssignModel(orderDetailId, beginTime) {
		...
		$(".dispatchOrder").css("display", "inline-block");
	...
	function openDeleteModel() {
		BootstrapDialog.confirm({
			...
			callback : function(result) {
				...
				if (result) {
					$.ajax({
						url : '${ctx}/order/cancelDispatch.htm',
						type : 'POST',//PUT DELETE POST
						data : {
							'orderDetailId' : $("#fmOrderDetailId").val(),
						},
						dataType : "json",
						success : function(data) {
							if (data.result == 1) {
